...
class InstructorDiscussionPostDetailScreen extends StatefulWidget {
  const InstructorDiscussionPostDetailScreen({
    super.key,
  ...
  @override
  State<InstructorDiscussionPostDetailScreen> createState() =>
      _InstructorDiscussionPostDetailScreenState();
}

class _InstructorDiscussionPostDetailScreenState
    extends State<InstructorDiscussionPostDetailScreen>
      ...
      builder: (context, themeState) {
        ...
        if (thread == null) {
          return Scaffold(
            ...
            body: InstructorDiscussionEmptyState(
              isDark: isDark,
      ...
      children: <Widget>[
        if (thread.isLocked)
          Container(
            ...
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
              color: InstructorColors.warningLight,
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                color: InstructorColors.warning.withValues(alpha: 0.3),
              ),
            ),
            child: Row(
              children: <Widget>[
                const Icon(Icons.lock_rounded, color: InstructorColors.warning),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
  ...
  Widget _buildDetailsTab(
    ...
    final info = <({String label, String value})>[
      (
        ...
        value: instructorDiscussionDisplayName(context, thread.createdByName),
      ),
      (
        label: l10n.course,
        value: _course == null
      ...
      (
        ...
        value: _course?.section.sectionNumber ?? '—',
      ),
      (
      ...
      (
        ...
        value: instructorDiscussionFormatDate(context, thread.createdAt),
      ),
      (
        label: l10n.instructorDiscussionUpdatedLabel,
        value: instructorDiscussionFormatDate(context, thread.updatedAt),
      ),
      (
        label: l10n.status,
          ...
          Row(
            children: <Widget>[
              Container(
                ...
                child: Icon(icon, color: InstructorColors.primary, size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
  ...
  Widget _buildHighlightedReply(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isDark
            ? InstructorColors.surfaceColor(isDark).withValues(alpha: 0.78)
            : const Color(0xFFF8FBFF),
        borderRadius: BorderRadius.circular(18),
        border: Border.all(
          color: InstructorColors.success.withValues(alpha: 0.28),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
            instructorDiscussionDisplayName(context, reply.userName),
            style: TextStyle(
              color: InstructorColors.textPrimaryColor(isDark),
              fontSize: 13,
              fontWeight: FontWeight.w800,
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: reply.isAnswer
                ? InstructorColors.success.withValues(alpha: 0.35)
            ...
            Row(
              ...
              children: <Widget>[
                CircleAvatar(
                  ...
                  child: Text(
                    instructorDiscussionInitials(reply.userName),
                    style: const TextStyle(
                      color: InstructorColors.primary,
                      fontSize: 12,
                      fontWeight: FontWeight.w800,
            ...
            Container(
              ...
              child: Row(
                children: <Widget>[
                  Expanded(
                    child: Text(
                      '${l10n.replyingTo}: ${instructorDiscussionDisplayName(context, _replyingTo!.userName)}',
                      style: const TextStyle(
                        color: InstructorColors.primary,
                        fontSize: 12,
                        fontWeight: FontWeight.w700,
