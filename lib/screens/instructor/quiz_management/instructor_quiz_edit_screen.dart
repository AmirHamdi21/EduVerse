...
class _EditState extends State<InstructorQuizEditScreen>
  ...
  void initState() {
    ...
    _timeLimit = TextEditingController(
      text: q.timeLimitMinutes?.toString() ?? '',
    );
  ...
  // ── Settings Tab ──────────────────────────────────────────────────────────
  Widget _buildSettingsTab(bool dk) => Form(
    key: _formKey,
    child: ListView(
      ...
      children: [
        _sec(dk, 'Basic Info', [
          _f(
            ...
            'Title *',
            _title,
            v: (s) => s == null || s.isEmpty ? 'Required' : null,
          ),
          const SizedBox(height: 14),
          _f(dk, 'Description', _desc, lines: 3),
          const SizedBox(height: 14),
          _f(dk, 'Instructions', _instructions, lines: 2),
          const SizedBox(height: 14),
        ...
        _sec(dk, 'Settings', [
          ...
          Row(
            children: [
              Expanded(
                child: _f(dk, 'Time Limit (min)', _timeLimit, num: true),
              ),
              const SizedBox(width: 12),
              Expanded(child: _f(dk, 'Max Attempts', _maxAttempts, num: true)),
            ],
          ),
          const SizedBox(height: 14),
          Row(
            children: [
              Expanded(
                child: _f(dk, 'Passing Score (%)', _passingScore, num: true),
              ),
              const SizedBox(width: 12),
              Expanded(child: _f(dk, 'Weight', _weight, num: true)),
            ],
          ),
          const SizedBox(height: 14),
          _tgl(
    ...
    return ListView(
      ...
      children: [
        ..._questions.asMap().entries.map(
          (e) => _buildQuestionCard(dk, e.key, e.value),
        ),
        const SizedBox(height: 12),
        // Add question button
        Material(
          ...
          child: InkWell(
            onTap: () => setState(
              () => _questions.add(_QuestionDraft(provenance: 'manual')),
            ),
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(14),
                border: Border.all(
                  color: InstructorColors.primary.withValues(alpha: 0.4),
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: TextStyle(
                    fontWeight: FontWeight.w600,
                    fontSize: 14,
                    color: dk ? Colors.white : const Color(0xFF1E293B),
                  ),
                ),
              ),
              if (q.provenance == 'ai')
          ...
          _dd(
            ...
            (v) => setState(() => q.type = QuestionTypeEnum.fromJson(v)),
          ),
          const SizedBox(height: 12),
          // Question text
          TextFormField(
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  onChanged: (v) => q.points = double.tryParse(v) ?? 10,
                  style: TextStyle(
                    color: dk ? Colors.white : const Color(0xFF1E293B),
                    fontSize: 14,
                  ),
          ...
          if (q.type == QuestionTypeEnum.multipleChoice ||
              q.type == QuestionTypeEnum.trueFalse) ...[
            const SizedBox(height: 12),
                ...
                child: Row(
                  children: [
                    Radio<int>(
                      value: e.key,
                      groupValue:
                          int.tryParse(q.correctAnswer?.toString() ?? '0') ?? 0,
                      onChanged: (v) =>
                          setState(() => q.correctAnswer = v.toString()),
                      activeColor: InstructorColors.primary,
                    ),
                    Expanded(
                      child: TextFormField(
                        initialValue: e.value['text']?.toString() ?? '',
                    ...
                    if (q.options.length > 2 &&
                      ...
                      IconButton(
                        icon: Icon(
                          Icons.close,
                          size: 16,
                          color: dk ? Colors.white38 : Colors.black26,
                        ),
                        onPressed: () =>
                            setState(() => q.options.removeAt(e.key)),
                      ),
                  ],
                ),
              ),
            ...
            if (q.type == QuestionTypeEnum.multipleChoice)
              TextButton.icon(
                onPressed: () => setState(() => q.options.add({'text': ''})),
          ...
          if (q.type == QuestionTypeEnum.shortAnswer) ...[
            const SizedBox(height: 12),
            TextFormField(
              initialValue: q.correctAnswer?.toString() ?? '',
          ...
          if (q.type == QuestionTypeEnum.matching) ...[
            ...
            const SizedBox(height: 8),
            ...q.matchingPairs.asMap().entries.map(
              (e) => Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Row(
                  children: [
                    Expanded(
                      child: TextFormField(
                        initialValue: e.value['left'] ?? '',
                        onChanged: (v) => setState(
                          () => q.matchingPairs[e.key] = {
                            'left': v,
                            'right': q.matchingPairs[e.key]['right'] ?? '',
                          },
                        ),
                        style: TextStyle(
                          color: dk ? Colors.white : const Color(0xFF1E293B),
                        ...
                        decoration: _inputDeco(dk, 'Left ${e.key + 1}'),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Icon(
                    ...
                    Expanded(
                      child: TextFormField(
                        initialValue: e.value['right'] ?? '',
                        onChanged: (v) => setState(
                          () => q.matchingPairs[e.key] = {
                            'left': q.matchingPairs[e.key]['left'] ?? '',
                            'right': v,
                          },
                        ),
                        style: TextStyle(
                        ...
                        decoration: _inputDeco(dk, 'Right ${e.key + 1}'),
                      ),
                    ),
                    IconButton(
                      icon: Icon(
                      ...
                      onPressed: () =>
                          setState(() => q.matchingPairs.removeAt(e.key)),
                    ),
                  ],
                ),
              ),
            ...
            TextButton.icon(
              onPressed: () => setState(
                () => q.matchingPairs.add({'left': '', 'right': ''}),
              ),
              icon: const Icon(Icons.add, size: 16),
              label: const Text('Add Pair', style: TextStyle(fontSize: 12)),
              style: TextButton.styleFrom(
                foregroundColor: InstructorColors.primary,
              ),
            ),
  ...
  Widget _buildSaveBar(bool dk) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border(
        top: BorderSide(color: dk ? Colors.white12 : const Color(0xFFE2E8F0)),
      ),
    ),
  ...
  Widget _buildCourseSelector(bool dk) {
    final bloc = context.read<InstructorCoursesBloc>();
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: dk ? Colors.white12 : const Color(0xFFE2E8F0),
            ),
          ),
          child: DropdownButtonHideUnderline(
            child: DropdownButton<int>(
              ...
              hint: Text(
                'Select course',
                style: TextStyle(
                  color: dk ? const Color(0xFF64748B) : const Color(0xFF94A3B8),
                  fontSize: 14,
                ),
              ),
              isExpanded: true,
  ...
  Widget _sec(bool dk, String t, List<Widget> ch) => Container(
  ...
  Widget _tgl(bool dk, String l, bool val, ValueChanged<bool> cb) => Row(
    children: [
      Expanded(
        child: Text(
          l,
