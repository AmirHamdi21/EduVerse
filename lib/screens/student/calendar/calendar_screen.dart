...
class InstructorColors {
  ...
  static Color background(bool isDark) =>
      StudentCoursesTheme.scaffoldBackground(isDark);
  ...
  static Color textSecondaryColor(bool isDark) =>
      StudentCoursesTheme.mutedText(isDark);
...
class _CalendarView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => _showAddEventSheet(context, isDark),
  ...
  Widget _buildHeroCard(
    ...
    final officeHours = state.filteredEvents
        .where((event) => event.type == EventType.lab)
        .length;
  ...
  Widget _buildControlMenus(
    ...
    final filterSummary = _activeFilterCount(state) == 5
        ? l10n.calendarAllEventTypes
        : '${_activeFilterCount(state)} ${l10n.events}';
    final courses = _courseOptions(state);
    final courseSummary = state.courseFilter ?? l10n.allCourses;
      ...
      children: [
        ...
        Expanded(
          child: _buildMenuButton(
            ...
            title: l10n.courses,
            subtitle: courses.isEmpty ? l10n.allCourses : courseSummary,
            color: InstructorColors.accent,
            isDark: isDark,
            onTap: (buttonContext) => _showCourseMenu(buttonContext, isDark),
          ),
        ),
      ],
    );
        ...
        children: views.map((view) {
          ...
          return Expanded(
            child: GestureDetector(
              onTap: () => context.read<CalendarCubit>().setViewType(view.$1),
      ...
      header: Row(
        children: [
          ...
          if (events.length > 3)
            TextButton(
              onPressed: () =>
                  _showDateEvents(context, state.selectedDate, events, isDark),
              child: Text(
                l10n.details,
                style: const TextStyle(fontWeight: FontWeight.w700),
              ),
  ...
  Widget _buildEventTile(
    ...
    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: () => _showEventDetails(context, event, isDark),
        borderRadius: BorderRadius.circular(22),
        child: Ink(
    ...
    showMenu<void>(
      ...
      items: [
        _buildMenuOption(
          ...
          onTap: () => cubit.toggleFilterType('lectures'),
        ),
        _buildMenuOption(
          ...
          onTap: () => cubit.toggleFilterType('labs'),
        ),
        _buildMenuOption(
          label: l10n.calendarCampusEvents,
          icon: Icons.campaign_rounded,
          color: InstructorColors.accent,
          isActive: cubit.state.filter.assignments,
          onTap: () => cubit.toggleFilterType('assignments'),
        ),
        _buildMenuOption(
          label: l10n.exam,
          ...
          onTap: () => cubit.toggleFilterType('exams'),
        ),
        _buildMenuOption(
          label: l10n.personalTaskType,
          icon: Icons.auto_awesome_rounded,
          color: InstructorColors.teal,
          isActive: cubit.state.filter.personalTasks,
          onTap: () => cubit.toggleFilterType('personalTasks'),
        ),
        PopupMenuItem<void>(
          height: 44,
          onTap: () => cubit.updateFilter(const EventFilter()),
          child: Text(
            l10n.all,
            style: const TextStyle(fontWeight: FontWeight.w700),
          ),
  ...
  void _showCourseMenu(BuildContext context, bool isDark) {
    final cubit = context.read<CalendarCubit>();
    final l10n = AppLocalizations.of(context);
    final courses = _courseOptions(cubit.state);
    ...
    showMenu<void>(
      ...
      items: [
        _buildMenuOption(
          ...
          icon: Icons.menu_book_rounded,
          color: InstructorColors.primary,
          isActive: cubit.state.courseFilter == null,
          onTap: () => cubit.setCourseFilter(null),
        ),
        ...courses.map(
          (course) => _buildMenuOption(
            label: course,
            icon: Icons.bookmarks_rounded,
            color: InstructorColors.accent,
            isActive: cubit.state.courseFilter == course,
            onTap: () => cubit.setCourseFilter(course),
          ),
        ),
      ],
    );
  ...
  void _showAddEventSheet(BuildContext context, bool isDark) {
    final cubit = context.read<CalendarCubit>();
  ...
  void _showDateEvents(
    ...
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => _DateEventsSheet(
        date: date,
        events: events,
        isDark: isDark,
        onEventTap: (event) {
  ...
  int _activeFilterCount(CalendarState state) {
    var count = 0;
    if (state.filter.lectures) count++;
    if (state.filter.labs) count++;
    if (state.filter.assignments) count++;
    if (state.filter.exams) count++;
    if (state.filter.personalTasks) count++;
    return count;
  }

  List<String> _courseOptions(CalendarState state) {
    final options =
        state.unifiedItems
