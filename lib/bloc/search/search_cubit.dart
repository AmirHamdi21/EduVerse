...
import '../assignments/assignments_cubit.dart';
...
class SearchCubit extends Cubit<SearchState> {
  ...
  static final List<_FeatureItem> _features = [
    _FeatureItem('Dashboard', '/dashboard', Icons.dashboard_rounded, 'Home dashboard overview'),
    _FeatureItem('Courses', '/courses', Icons.school_rounded, 'Browse enrolled courses'),
    _FeatureItem('AI Quiz', '/ai-quiz-generator', Icons.quiz_rounded, 'Generate AI quizzes'),
    _FeatureItem('Flashcards', '/flashcards', Icons.style_rounded, 'Study with flashcards'),
    _FeatureItem('Tasks', '/tasks', Icons.task_alt_rounded, 'Manage your tasks'),
    _FeatureItem('Labs', '/labs', Icons.science_rounded, 'Lab sessions'),
    _FeatureItem('Assignments', '/assignments', Icons.assignment_rounded, 'View assignments'),
    _FeatureItem('Grades', '/grades', Icons.grade_rounded, 'Academic performance'),
    _FeatureItem('Grade Analysis', '/grade-analysis', Icons.analytics_rounded, 'Performance insights'),
    _FeatureItem('Calendar', '/calendar', Icons.calendar_month_rounded, 'Schedule and events'),
    _FeatureItem('Messages', '/messages', Icons.chat_rounded, 'Chat messages'),
    _FeatureItem('Notifications', '/notifications', Icons.notifications_rounded, 'Your notifications'),
    _FeatureItem('AI Chat', '/ai-chat', Icons.smart_toy_rounded, 'AI assistant'),
    _FeatureItem('AI Notes', '/ai-notes', Icons.note_alt_rounded, 'AI-powered notes'),
    _FeatureItem('Voice to Text', '/voice-to-text', Icons.mic_rounded, 'Voice recording'),
    _FeatureItem('Summarizer', '/summarizer', Icons.summarize_rounded, 'Text summarizer'),
    _FeatureItem('Smart Study', '/smart-study', Icons.psychology_rounded, 'Smart study plans'),
    _FeatureItem('My Files', '/my-files', Icons.folder_rounded, 'File management'),
    _FeatureItem('Attendance', '/attendance', Icons.fact_check_rounded, 'Attendance records'),
    _FeatureItem('Leaderboard', '/gamification', Icons.leaderboard_rounded, 'Gamification & rewards'),
    _FeatureItem('Profile', '/profile', Icons.person_rounded, 'Your profile'),
    _FeatureItem('Settings', '/settings', Icons.settings_rounded, 'App settings'),
  ];
  ...
  List<SearchResultItem> _searchAssignments(BuildContext context, String query) {
    try {
      final cubit = context.read<AssignmentsCubit>();
      final aState = cubit.state;
  ...
  List<SearchResultItem> _searchNotifications(BuildContext context, String query) {
    try {
  ...
  List<SearchResultItem> _applyFilters(List<SearchResultItem> results) {
    ...
    if (f.dateFrom != null) {
      filtered = filtered.where((r) =>
          r.date == null || r.date!.isAfter(f.dateFrom!)).toList();
    }
    if (f.dateTo != null) {
      filtered = filtered.where((r) =>
          r.date == null || r.date!.isBefore(f.dateTo!.add(const Duration(days: 1)))).toList();
    }
  ...
  void clearFilters(BuildContext context) {
    setFilter(
      SearchFilter(category: state.filter.category),
      context,
    );
  }
