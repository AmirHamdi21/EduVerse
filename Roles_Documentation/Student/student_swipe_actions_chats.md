# Swipe Actions Chats — Student Feature Documentation

## 1. Feature Summary
The **Swipe Actions Chats** capability belongs to the **Student** role and is grouped under **Profile & Settings**. This document captures implementation intent, navigation, architecture touchpoints, and operational expectations for maintainable evolution.

## 2. Access and Navigation
| Item | Details |
|---|---|
| Role Access | Student |
| Primary Route | `/settings/swipe-actions/chats` |
| Primary Screen | `lib/screens/student/settings/chat_swipe_settings_screen.dart` |
...
## 3. User Goals
- Complete key **swipe actions chats** tasks with minimal friction.
- Understand current state clearly (loading, success, empty, error).
- Preserve workflow continuity with adjacent role features.

## 4. Functional Scope
- Provides dedicated UI and actions for **Swipe Actions Chats**.
...
## 5. Primary User Flow
1. User opens `/settings/swipe-actions/chats`.
...
- `lib/config/app_router.dart`
- `lib/screens/student/settings/chat_swipe_settings_screen.dart`
