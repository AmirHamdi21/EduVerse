# Instructor Dashboard — Instructor Feature Documentation

## 1. Feature Summary
The **Instructor Dashboard** capability belongs to the **Instructor** role and is grouped under **Dashboard & Navigation**. This document captures implementation intent, navigation, architecture touchpoints, and operational expectations for maintainable evolution.

## 2. Access and Navigation
| Item | Details |
|---|---|
| Role Access | Instructor |
| Primary Route | `/instructor/dashboard` |
| Primary Screen | `lib/screens/instructor/dashboard/instructor_dashboard_screen.dart` |
| Feature Category | Dashboard & Navigation |
...
## 3. User Goals
- Complete key **instructor dashboard** tasks with minimal friction.
- Understand current state clearly (loading, success, empty, error).
- Preserve workflow continuity with adjacent role features.

## 4. Functional Scope
- Provides dedicated UI and actions for **Instructor Dashboard**.
...
## 5. Primary User Flow
1. User opens `/instructor/dashboard`.
...
|---|---|
| State Management | instructor_dashboard_cubit, instructor_courses_bloc |
| Service/API Layer | course_service.dart, student_stats_service.dart |
| Key Data Shapes | dashboard KPIs, instructor course summaries |
...
- Expose quick actions to high-frequency instructor tasks.
- Keep dashboard metrics aligned with role scope.
- Fail safely when non-critical widgets cannot load.
...
- **Current Notes:** Dashboard shell is complete; some cards use lightweight placeholder metrics.
...
- `lib/config/app_router.dart`
- `lib/screens/instructor/dashboard/instructor_dashboard_screen.dart`
